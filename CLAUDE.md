...
阶段五（行号槽 + 交互打磨）完成并 F5 验证：**行号槽**完成并 F5 验证——内核导出配置好的 `mdIt`，host 侧平行解析取每块 `token.map` 源码起始行、与顶层节点 1:1 zip，绝对定位兄弟层按 `coordsAtPos` 对齐块顶渲染（无竖线、右对齐、逐块标首行、rAF 节流）；源码物理行号口径与 .md 真实行号一致 `[D5-1]`。**查找高亮**完成并 F5 验证——webview 内自建查找框（原生 find 够不到 iframe），`Ctrl/Cmd+F` 开/`Esc` 关，setMeta 驱动内核 `searchHighlightPlugin`（内核零改动），Aa/.* 选项 + `n/m` 计数 + ↑↓·Enter 循环导航滚到 match `[D5-2]`。**空态/加载态**完成并 F5 验证——空态用内核 `placeholderPlugin`（未重做）；加载态 host HTML 预置 `Loading…` 占位、`init` 到达自动抹掉；`customEditors.priority` 改 `default`，`.md` 直接进 vsinkore `[D5-3]`。**Bubble Toolbar** 完成并 F5 验证——内核 `bubbleToolbarPlugin` 给选区几何 + `calcBubblePosition` 定位，host 自写浮层接 command（内核零改动），UI 严格复刻 `原型/editor-mockup.html`；文字色单色（swatch 显 `charts-green`、入档固定 `#4ec9b0`）；VSCode `fadeIn` 浮出 + 自绘 hover tooltip（通用 `tooltip.ts`，查找框共用）`[D5-4]`。期间修复含代码块文档里 bubble 延迟 0.4~0.8s 的 bug——根因是语法语言逐个异步加载各触发 RECHECK、每次同步全量 tokenize 阻塞主线程挤后 bubble 定时器；修法 ① tokenize 缓存治本 + ② rAF 合并 RECHECK 削峰 `[D5-bug]`。**右键菜单**完成——host 自建单层菜单（格式/插入/编辑），按内核 `isInTable` 分发：表格外弹格式菜单（含插表浮层）、表格内弹行/列/单元格编辑菜单（`[D5-5]`）；表格内选区另弹精简 Bubble（仅行内格式 7 项，`[D5-6]`）；链接 Ctrl/Cmd+点击经 `openLink` 消息走 extension `openExternal`（仅 http(s)）。文字标色由紫改绿、任务列表打勾框对齐与勾选态上色修复。整体验证 F5 跑通真实文档，期间修复行号空段落错位（`[D5-1]` 修正）、KaTeX 公式 CSP（`style-src` 开 `unsafe-inline`，取代 `[D0-8]` 口径，`[D5-7]`）、公式框 resize 白块、表格子浮层窗口溢出共 4 处。阶段五完结。
